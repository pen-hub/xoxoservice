...
export default function OrderDetailPage() {
    ...
    useEffect(() => {
        if (orderCode) {
            const unsubscribe = AppointmentService.onSnapshot(
                (allAppointments) => {
        ...
        try {
            ...
            if (
                ...
                try {
                    ...
                    console.error(
                        "Failed to create follow-up schedules:",
                        followUpError,
                    );
            ...
            if (
                ...
                try {
                    ...
                    console.error(
                        "Failed to send order confirmation message:",
                        msgError,
                    );
    ...
    const getAvailableStatusOptions = (currentStatus: OrderStatus) => {
        ...
        return [
            ...
            },
            {
                ...
                label: "Hoàn tiền",
                disabled:
                    currentStatus === OrderStatus.REFUND || currentIndex < 1, // Chỉ enable nếu đã CONFIRMED trở lên và chưa REFUND
            },
            {
                                    ...
                                    !canConfirmOrder.canConfirm && (
                                        <Alert
                                            ...
                                            description={
                                                <div>
                                                    <Text strong>
                                                        Vui lòng hoàn thành các
                                                        điều kiện sau:
                                                    </Text>
                                                    <ul className="mt-2 mb-0 pl-4">
                                                        {canConfirmOrder.reasons.map(
                                                            (reason, index) => (
                                                                <li key={index}>
                                                                    {reason}
                                                                </li>
                        ...
                        {orderStatus === OrderStatus.ON_HOLD &&
                            onImageUpload && (
                                <div className="mt-4 p-4 bg-orange-50 rounded-lg border border-orange-200">
                                    <Text
                                        strong
                                        className="text-orange-700 block mb-3"
                                    >
                                        📸 Tải ảnh sản phẩm đã hoàn thành
                                    </Text>
                                    <Text className="text-orange-600 text-sm block mb-3">
                                        Vui lòng tải lên ảnh sản phẩm sau khi
                                        hoàn thành để cập nhật cho khách hàng.
                                    </Text>
                                    <Upload
                                        multiple
                                        accept="image/*"
