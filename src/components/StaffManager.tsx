...
export default function StaffManager() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingStaff) {
        await updateStaff(firebaseApp, editingStaff.id, values);
  ...
  const handleDelete = async (staffId: string) => {
    try {
      await deleteStaff(firebaseApp, staffId);
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm p-6">
      ...
      <Modal
        ...
        width={500}
        okText={editingStaff ? "Cập nhật" : "Tạo mới"}
        cancelText="Hủy"
      >
