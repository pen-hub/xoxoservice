...
const KanbanBoard: React.FC<KanbanBoardProps> = ({
    ...
    const { query, updateQuery, updateQueries, applyFilter, reset } =
        useFilter();
        ...
        const membersMap: Record<string, IMembers> = membersData.reduce(
            (acc: Record<string, IMembers>, item: any) => {
                acc[item.id] = {
                    ...
                    date_of_birth:
                        item.data?.date_of_birth || item.date_of_birth || "",
                };
                return acc;
            },
            {},
    ...
    const filteredOrders = useMemo(() => {
        ...
        if (
            query["search,memberName"] &&
            query["search,memberName"] !== "all"
        ) {
    ...
    const statusSummary = useMemo(() => {
        ...
        filteredOrders.forEach((order) => {
            if (order.status && initialCounts[order.status] !== undefined) {
                    ...
                    <DragDropContext onDragEnd={onDragEnd}>
                        <div className="flex gap-3 min-w-max">
                            {columnsKanban.map((column) => {
                                const columnOrders = getOrdersForColumn(
                                    column.status,
                                );
                                ...
                                return (
                                    ...
                                    >
                                        <div
                                            className="rounded-lg p-3 h-[600px] w-full border flex flex-col"
                                            style={{
